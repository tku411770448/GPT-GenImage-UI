...
class UnifiedMainWindow(QMainWindow):
    ...
    def _state_path(self) -> Path:
        return ensure_dir(self.root / "_launcher_state") / "launcher_state.json"

    def _load_last_mode(self) -> str:
        try:
            data = json.loads(self._state_path().read_text(encoding="utf-8"))
            mode = str(data.get("active_mode") or "defect").lower()
            return mode if mode in {"defect", "food"} else "defect"
        except Exception:
            return "defect"

    def _save_last_mode(self, mode: str) -> None:
        try:
            self._state_path().write_text(json.dumps({"active_mode": mode}, ensure_ascii=False, indent=2), encoding="utf-8")
        except Exception:
            pass
