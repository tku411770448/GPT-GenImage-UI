...
def find_reference(image: Path, reference_dir: Path) -> Path | None:
    """Locate the pre-rendered ROI/Target annotation reference (Image 2) for a raw image.

    Reference images are paired one-to-one with raw images by file stem."""
    candidates = [reference_dir / f"{image.stem}{ext}" for ext in SUPPORTED]
    candidates += [reference_dir / f"{image.stem}_reference{ext}" for ext in SUPPORTED]
    candidates += [reference_dir / f"{image.stem}_ref{ext}" for ext in SUPPORTED]
    for p in candidates:
        if p.exists():
            return p
    return None
...
def main() -> None:
    ...
    if not args.dry_run:
        try:
            ...
            raise SystemExit(
                ...
                f"  {sys.executable} -m pip install -r {root.parent.parent / 'requirements.txt'}\n"
    ...
    # New layout: data/raw_image (Image 1) + data/reference_image (pre-rendered Image 2).
    # When --reference-dir is provided, reference-guided-edit pairs each raw image with its
    # reference and no longer needs separate ROI/Target masks.
    use_reference_dir = bool(args.reference_dir) and args.workflow == "reference-guided-edit"
    ...
    needs_reference = use_reference_dir
    needs_mask = (args.workflow in {"repair-and-random-generate", "repair-only"}
                  or (args.workflow == "generate-only" and args.placement_mode == "fixed-mask")
                  or (args.workflow == "reference-guided-edit" and not use_reference_dir))
    needs_target = (args.workflow == "repair-and-random-generate"
                    or (args.workflow == "generate-only" and args.placement_mode == "random-in-target")
                    or (args.workflow == "reference-guided-edit" and not use_reference_dir))
    ...
    for img in images:
        ...
        reference = find_reference(img, args.reference_dir) if (use_reference_dir and args.reference_dir.exists()) else None
        if needs_reference and reference is None:
            missing_reference.append(img.name)
        elif needs_mask and mask is None:
    ...
    if missing_reference:
        raise SystemExit(
            f"reference-guided-edit needs one reference image (Image 2) per raw image. Missing for {len(missing_reference)} image(s) in {args.reference_dir}:\n"
            + "\n".join(missing_reference[:20])
            + "\nPlease draw ROI / Target Area for every selected image so its reference image is rendered."
        )
    if missing:
    ...
    for index in range(start_index, total_jobs):
        ...
        if reference is not None:
            cmd += ["--annotation-reference", str(reference)]
        if mask is not None:
            cmd += ["--mask", str(mask)]
