...
Workflows:
  reference-guided-edit        clean image + annotation reference image + prompt; no API mask
...
def load_user_prompt(defect_type: str | None, prompt_file: Path | None, inline_prompt: str | None, size: str) -> dict[str, str]:
    ...
    the ChatGPT UI style: unannotated original image + annotation reference image + natural-language prompt.
...
def strip_region_coordinate_lines(text: str) -> str:
    """Remove UI coordinate dumps from older saved prompt files.

    The reference-guided workflow provides location through Image 2, so sending
    x/y coordinates in the text prompt is intentionally avoided.
    """
    kept: list[str] = []
    coord_line = re.compile(r"^\s*(?:\[[^\]]+\]\s*)?(?:ROI\d*|ROI_\d+|Target\s*Area\d*|target_area(?:_\d+)?)\s*:", re.I)
    for line in str(text or "").splitlines():
        raw = line.strip()
        if coord_line.match(raw):
            continue
        if re.search(r"\bpolygon\s+closed\s+points\b", raw, flags=re.I):
            continue
        if re.search(r"\bx1\s*[：:]\s*\d+.*\by1\s*[：:]\s*\d+", raw, flags=re.I):
            continue
        kept.append(line)
...
def reference_guided_system_prompt(defect_type: str) -> str:
    defect_type = sanitize_defect_type(defect_type)
    return (
        "\n\nReference-guided editing protocol:\n"
        "- You are given two input images. Image 1 is the unannotated original image and is the only editable visual source.\n"
        "- Image 2 is an annotation reference image only. Use it only to understand ROI and Target Area locations.\n"
        "- Never copy, reproduce, preserve, imitate, or hallucinate any annotation marks from Image 2.\n"
        "- Treat magenta/red ROI markings in Image 2 as source/remove regions. Treat cyan/blue Target Area markings in Image 2 as the only allowed placement regions.\n"
        "- Do not rely on textual x/y coordinates; the spatial guidance is provided by Image 2.\n"
        f"- Defect class: `{defect_type}`. Remove the ROI defect(s) and generate the new `{defect_type}` target(s) only inside the Target Area.\n"
        "- Preserve Image 1 globally: same background, material, lighting, camera angle, perspective, crop, and resolution.\n"
        "- Output must contain no ROI boxes, Target Area boxes, colored lines, labels, text, arrows, coordinates, masks, UI marks, square patch artifacts, or global restyling."
    )

def build_prompt_bundle(cfg: dict[str, str]) -> dict[str, str]:
    defect_type = cfg.get("defect_type", "defect")
    user_prompt = strip_region_coordinate_lines(cfg["user_prompt"].strip())
    # Reference-guided mode: unannotated source image + separate annotation reference
    # image.  No ROI / Target coordinates are appended to the text prompt, and
    # no API mask is used.
    reference_guided = user_prompt + reference_guided_system_prompt(defect_type)
    # Classic prompt-only mode, kept for compatibility.
    base_note = (
        "\n\n補充要求：請把輸入圖像視為待編輯的工業瑕疵圖。"
        "若另有標註參考圖，僅作為定位參考，最終輸出不可保留框線或文字。"
    )
    ...
    return {
        "reference_guided_edit_prompt": reference_guided,
        ...
        "full_workflow_prompt": reference_guided,
...
def _mask_outline(mask: Image.Image, width: int = 3) -> Image.Image:
    m = binarize(mask)
    if width <= 1:
        dilated = m
    else:
        dilated = m.filter(ImageFilter.MaxFilter(width * 2 + 1))
    eroded = m.filter(ImageFilter.MinFilter(max(3, width * 2 - 1)))
    return ImageChops.difference(dilated, eroded).point(lambda v: 255 if v > 0 else 0)


def make_annotation_reference(
    image: Image.Image,
    roi_mask: Image.Image | None = None,
    target_area: Image.Image | None = None,
) -> Image.Image:
    """Create Image 2 for the reference-guided workflow.

    This file is never used as the editable source.  It is passed as a second
    input image so GPT Image can infer ROI / Target positions visually without
    receiving textual coordinates or an API mask.
    """
    base = image.convert("RGBA")
    w, h = base.size
    overlay = Image.new("RGBA", base.size, (0, 0, 0, 0))

    def paste_colored(mask: Image.Image | None, color: tuple[int, int, int], fill_alpha: int, outline_alpha: int) -> None:
        nonlocal overlay
        if mask is None:
            return
        m = binarize(mask)
        ...
        fill.putalpha(m.point(lambda v: fill_alpha if v > 0 else 0))
        overlay = Image.alpha_composite(overlay, fill)
        outline = _mask_outline(m, width=max(2, int(round(min(w, h) / 450))))
        line = Image.new("RGBA", base.size, (*color, 0))
        line.putalpha(outline.point(lambda v: outline_alpha if v > 0 else 0))
        overlay = Image.alpha_composite(overlay, line)

    paste_colored(target_area, (0, 205, 255), 28, 255)
    paste_colored(roi_mask, (255, 23, 68), 58, 255)

    out = Image.alpha_composite(base, overlay)
    draw = ImageDraw.Draw(out)
    # A tiny legend is useful for the model, but the prompt explicitly forbids
    # copying it into Image 1's output. Keep it in the corner to avoid covering
    # the edge regions.
    legend = "Image 2 reference only | ROI=magenta remove | Target=cyan generate"
    try:
        bbox = draw.textbbox((0, 0), legend)
        tw, th = bbox[2] - bbox[0], bbox[3] - bbox[1]
    except Exception:
        tw, th = (min(w - 12, len(legend) * 6), 14)
    pad = 6
    draw.rectangle((pad, pad, min(w - pad, pad + tw + 10), pad + th + 10), fill=(255, 255, 255, 215))
    draw.text((pad + 5, pad + 5), legend, fill=(0, 0, 0, 255))
...
def parse_args():
    ...
    p.add_argument("--image", type=Path, required=True, help="Input image path. For reference-guided mode this must be the clean original image.")
    p.add_argument("--annotation-reference", type=Path, default=None, help="Optional ROI/Target annotation reference image for reference-guided-edit. If omitted, one is rendered from --mask and --target-area.")
    p.add_argument("--mask", type=Path, default=None, help="Optional source/repair mask. In reference-guided-edit it is used only to render Image 2, not sent as an API mask.")
...
def main():
    ...
    if args.workflow == "reference-guided-edit" and args.annotation_reference is None and (args.mask is None or args.target_area is None):
        raise ValueError("reference-guided-edit requires either --annotation-reference or both --mask and --target-area to render Image 2.")
    ...
    annotation_reference_preview_path: Path | None = None
    if args.workflow == "reference-guided-edit":
        if args.annotation_reference is not None and args.annotation_reference.exists():
            annotation_reference_preview_path = args.annotation_reference
        elif repair_mask is not None and target_area is not None:
            annotation_reference_preview_path = run_dir / "annotation_reference_preview.png"
            make_annotation_reference(image, repair_mask, target_area).save(annotation_reference_preview_path)
    ...
    metadata: dict[str, Any] = {
        ...
        "annotation_reference": str(annotation_reference_preview_path) if annotation_reference_preview_path else (str(args.annotation_reference) if args.annotation_reference else None),
        "reference_guided_mode": args.workflow == "reference-guided-edit",
    ...
    for i in range(int(args.num_outputs)):
        ...
        if args.workflow == "reference-guided-edit":
            print("[INFO] reference-guided edit: Image 1 clean original + Image 2 annotation reference, no API mask")
            reference_images: list[Image.Image] = []
            annotation_source = None
            if args.annotation_reference is not None and args.annotation_reference.exists():
                reference_images.append(Image.open(args.annotation_reference).convert("RGB"))
                annotation_source = str(args.annotation_reference)
            else:
                annotation_img = make_annotation_reference(current_image, repair_mask, target_area)
                annotation_path = run_dir / f"annotation_reference_seed{seed}.png"
                annotation_img.save(annotation_path)
                reference_images.append(annotation_img)
                annotation_source = str(annotation_path)
            metadata.setdefault("annotation_references", []).append({"seed": seed, "path": annotation_source})
            generated, api_meta = _call_openai_image_edit(
                client,
                current_image,
                bundle["reference_guided_edit_prompt"],
                args,
                run_dir,
                f"reference_guided_seed{seed}",
                edit_mask=None,
            ...
            api_meta["reference_guided_policy"] = "Image 2 was passed as annotation reference only; no OpenAI API mask or textual coordinates were sent."
            ...
            metadata.setdefault("per_seed", []).append({
                ...
                "annotation_reference": annotation_source,
...
def cleanup_run_dir(run_dir: Path, final_outputs: list[str], keep_intermediates: bool = False) -> None:
    ...
    keep: set[Path] = {
        ...
        run_dir / "prompt_used.txt",
        run_dir / "log.txt",
        # Preserve source and masks so Step 9 can generate true per-image labels
        # by comparing generated outputs against the original image inside Target Area.
        run_dir / "input.png",
        run_dir / "target_area.png",
        run_dir / "original_repair_mask.png",
        run_dir / "prototype_mask.png",
    }
