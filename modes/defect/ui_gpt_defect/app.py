...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.projects_root = ensure_dir(self.root / "project_defect")
    ...
    def mode_projects_root(self, mode: str) -> Path:
        ...
        folder_name = "project_food" if mode == "food" else "project_defect"
    ...
    def delete_all_crops(self) -> None:
        if not (hasattr(self, "crop_done_grid") and self.crop_done_grid.count()):
            ...
            return
        # Remove every Step 4 input crop together with its ROI / Target Area, mask, and
    ...
    def start_generation(self, resume: bool=False)->None:
        ...
        for p in selected_paths:
            if not (self.masks_dir()/f"{p.stem}_mask.png").exists() or not (self.target_masks_dir()/f"{p.stem}_target_area.png").exists():
                missing_ref.append(p.name)
        if missing_ref:
            QMessageBox.warning(self,"Missing", "新版參考圖模式需要每張選定圖像都有 ROI 與 Target Area 標註，才能產生 Image 2 參考圖。缺少：\n" + "\n".join(missing_ref[:10])) ; return
