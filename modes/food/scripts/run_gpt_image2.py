...
def sanitize_defect_type(name: str | None) -> str:
    if name is None:
        return "custom"
    # Preserve Unicode (e.g. CJK class names) while keeping the value safe to use as a
    # path component. Only collapse whitespace and strip characters that are illegal in
    # Windows/Unix file names -- do NOT drop non-ASCII letters, which would wipe out a
