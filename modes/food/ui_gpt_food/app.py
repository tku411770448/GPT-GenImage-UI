...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.setWindowTitle("GPT GenImage UI - Gen Food")
    ...
    def _class_input_dirs_for_state(self, state: UIState | None = None) -> list[tuple[Path, Path]]:
        ...
        for pdir in self.candidate_project_dirs(st):
            # New flat food layout: data/ holds both raw and inputs; regions are unused.
            flat_data = pdir / "data"
            flat_key = f"{flat_data}|{flat_data}"
            if flat_key not in seen:
                seen.add(flat_key); dirs.append((flat_data, flat_data))
    ...
    def raw_dir(self) -> Path:
        # Food uses a flat data/ folder: raw and inputs are the same place.
    ...
    def masks_dir(self) -> Path:
        # Food's prompt-only-edit never uses masks; point at a hidden, non-created path.
        return self.project_dir() / "data" / "_unused"

    def target_masks_dir(self) -> Path:
        # Food's prompt-only-edit never uses target masks; hidden, non-created path.
        return self.project_dir() / "data" / "_unused"
    ...
    def wrap_page(self, step_idx: int, content_layout: QVBoxLayout) -> QWidget:
        ...
        if step_idx not in {0, 2, 3, 5, 7, 9}:
    ...
    def refresh_project_list(self) -> None:
        if not hasattr(self, "project_cards_layout"):
            return
        # Preserve the homepage scroll position across the card grid rebuild so
        # selecting/switching projects does not jump the view to the top.
        sb = self.project_cards_scroll.verticalScrollBar() if hasattr(self, "project_cards_scroll") else None
    ...
    def submit_aggregate(self) -> bool:
        ...
        self.state.aggregate_summary = summary
    ...
    def load_crop_record(self, crop_path: Path) -> dict:
        # Food no longer writes crop records (crop is done in place under data/).
        return {}
    ...
    def use_selected_original_image(self) -> None:
        ...
        self.refresh_region_thumbs(); self.refresh_prompt_groups(); self.update_step_buttons()
        self.status_label.setText(f"Status: 已將原始圖片加入 Step 4 輸入圖像：{src.name} ({w}*{h})")
    ...
    def use_original_images_without_crop(self, confirm: bool = True, mark_completed: bool = False, auto_select: bool = False) -> bool:
        ...
        if not raws:
            ...
            return False
        # Only warn when there are previously cropped Step 3 inputs that this action would
        # discard. A clean / first-time project (no cropped inputs) switches silently.
        if confirm and ("crop" in self.step4_input_modes()):
            ret = QMessageBox.question(
                self,
                "直接使用原圖",
                "此動作會把所有原圖直接做為 Step 4 使用。既有的 runs 會保留。是否繼續？",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            )
            if ret != QMessageBox.StandardButton.Yes:
                return False
    ...
    def _renumber_crop_outputs(self) -> None:
        """No-op for food: crops are done in place and there are no crop records
        or ROI/Target masks to keep in sync."""
        return
    ...
    def delete_selected_crop(self) -> None:
        ...
        for item in items:
            ...
            try:
                ...
                pass
            # Remove from any stored selection state.
            try:
                self.state.selected_region_stems = [s for s in getattr(self.state, "selected_region_stems", []) if s != stem]
            except Exception:
                pass
    ...
    def save_current_regions(self, silent: bool=False) -> None:
        # Food's prompt-only-edit workflow does not use ROI/Target masks or region
        # files. Keep the method functional for the Step 4 UI but do not write any
    ...
    def on_prompt_mode_changed(self) -> None:
        ...
        for name in ["prompt_template_label", "prompt_template_combo", "apply_template_btn"]:
            if hasattr(self, name):
                getattr(self, name).setVisible(is_template)
        self.update_actual_prompt_preview()
        self.mark_dirty(5)
    ...
    def build_actual_prompt(self) -> str:
        ...
        return (self.state.prompt_input or "").strip()
    ...
    def save_prompt(self, show_message: bool=True) -> bool:
        self.update_actual_prompt_preview()
        self.update_state_from_widgets()
        text = self.prompt_edit.toPlainText().strip() if hasattr(self, "prompt_edit") else ""
        if not text:
            QMessageBox.warning(self,"Missing","Prompt 不能空白。") ; return False
        # The prompt actually sent equals the 輸入指令 text.
        self.state.prompt_input = self.prompt_edit.toPlainText() if hasattr(self, "prompt_edit") else ""
        self.save_state()
    ...
    def start_generation(self, resume: bool=False)->None:
        ...
        self.save_state()
        self.log_event(f"generation start: project={self.state.project_id} total={self.expected_generation_steps()} resume={resume}")
            ...
            try:
                ...
                if label == "generation":
                    ...
                    if code != 0:
                        ...
                        self.save_state(); self.update_step_buttons()
                        self.log_event(f"generation failed: code={int(code)}")
